...
**pf4j-spring-boot-starter** is a Spring Boot starter that integrates **pf4j** for applications using pf4j. It provides auto-configuration, property binding, and ready-to-use beans so that applications can consume pf4j capabilities with minimal setup.

| Dimension | Description |
|---|---|
| Type | Spring Boot Starter |
| Consumers | Spring Boot applications using pf4j |
| Core Capabilities | auto-configuration, property binding, ready-to-use beans for pf4j |
| JDK | `17` |
| Coordinates | `io.github.easy4j:pf4j-spring-boot-starter:1.0.2.RELEASE` |
| Config Prefix | `pf4j` |
...
| Property Binding | ✅ Stable | Binds `pf4j.*` to `Pf4jProperties` |
| `PluginStateListener` bean | ✅ Stable | Auto-registered via Pf4jAutoConfiguration |
...
| `ExtensionsInjector` | classpath + property | not created |
| `ControllerExtensionsInjector` | classpath + property | not created |
| `PluginManager` | classpath + property | not created |
| `UpdateRepositoryProvider` | classpath + property | not created |
...
```java
@Autowired
private PluginStateListener pluginStateListener;
```
...
|---|---|---|:---:|---|:---:|
| `pf4j.enabled` | boolean | `true` | No | Enable the starter | No |
